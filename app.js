...
(function () {
  ...
  let balls = [
    {ball:1,lat:40.44361383246247,lon:-3.6665857235848986,dist:null,x:null,y:null,recogida:false,px:null,py:null},
    {ball:2,lat:null,lon:null,dist:null,x:null,y:null,recogida:true,px:null,py:null}
  ];

  var click = new Audio('clickradar.mp3');
  click.play();
  var shortbeep = new Audio('shortbeep2.mp3');
  //audio.loop = true;
  ...
  // Get the Object by ID
	...
	//var svgDoc = rose.contentDocument;
	// Get one of the SVG items by ID;
	//var ball1 = svgDoc.getElementById("ball");
  ...
  var positionLatraw = document.getElementById("position-latraw");
  var positionLngraw = document.getElementById("position-lngraw");
  ...
  function locationUpdate(position) {
    ...
    for ( var i=0; i<balls.length;i++){
      if (!balls[i].recogida) {
        balls[i].x = (balls[i].lon-positionCurrent.lng) * Math.cos(balls[i].lat)*(40000/360);
        balls[i].y = (balls[i].lat-positionCurrent.lat) * (10.000/90);
        balls[i].dist = Math.sqrt(Math.pow(balls[i].x,2) + Math.pow(balls[i].y,2))*1000;
        console.log(balls[i]);
//3pixeles equivalen a 1m
        if (balls[i].dist < 62 ) {
          ...
          ball1.setAttribute("r", "2");
          shortbeep.loop=true;
          shortbeep.play();
        }
        else{
          ball1.setAttribute("stroke", "green");
          ball1.setAttribute("fill", "green");
          ball1.setAttribute("fill", "green");
          ball1.setAttribute("r", "0");
        }
      };
    }
console.log(balls[0]);
      ball1.setAttribute("cx", balls[0].px);
      ball1.setAttribute("cy", balls[0].py);

      fileToText("my-file.txt", (text) => {
        console.log(text);
  });
  console.log(leido);
  ...
  btnNightmode.addEventListener("click", playaudio);
  //btnMap.addEventListener("click", playaudio);
